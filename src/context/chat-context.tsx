...
interface ChatContextType {
  ...
  hasConversation: (chat?: CurrentChat) => boolean;
...
export const ChatProvider: React.FC<ChatProviderProps> = ({ children }) => {
  ...
  const [chatCache, setChatCache] = useState<Map<string, ChatSessionDetail>>(
    new Map()
  );
      ...
      try {
        ...
        if ("isTemp" in currentChat && currentChat.isTemp) {
          ...
          const chatMessage = {
            ...
            role:
              typeof userMessage.role === "number"
                ? userMessage.role
            ...
            tokenCount: 0,
            timestamp:
              typeof userMessage.timestamp === "string"
                ? userMessage.timestamp
                : (userMessage.timestamp as Date).toISOString(),
          };
          updatedChat = {
            ...currentChat,
        ...
        if (("isTemp" in currentChat && currentChat.isTemp) || !sessionId) {
          ...
          const chatMessages = (
            chatWithoutTemp.messages as TempChatMessage[]
          ).map((msg: TempChatMessage) => ({
            id: msg.id,
            content: msg.content,
            role:
              typeof msg.role === "number"
                ? msg.role
                : msg.role === "user"
                ? 1
                : 2,
        ...
        setCurrentChat((prev) => {
          ...
          if ("isTemp" in prev && prev.isTemp) {
            ...
            const chatMessage = {
              ...
              tokenCount: 0,
              timestamp:
                typeof placeholderAssistantMessage.timestamp === "string"
                  ? placeholderAssistantMessage.timestamp
                  : (
            ...
            setCurrentChat((prev) => {
              ...
              if (
                ...
                if ("isTemp" in prev && prev.isTemp) {
                  ...
                  if (prev.id) {
                    setChatCache((cache) =>
                      new Map(cache).set(prev.id!, updatedChat)
                    );
                  }
                  return updatedChat;
                }
  ...
  // Helper function to check if a chat has conversation (at least one user-assistant pair)
  const hasConversation = useCallback(
    (chat?: CurrentChat) => {
      ...
      if (
        !targetChat ||
        !targetChat.messages ||
        targetChat.messages.length < 2
      ) {
        return false;
      }

      // Check if there's at least one complete conversation (user message + AI response)
      const messages = targetChat.messages;
      let hasUserMessage = false;

      for (const message of messages) {
        const role =
          typeof message.role === "number" ? message.role : message.role;

        if (role === "user" || role === 1) {
          hasUserMessage = true;
        } else if ((role === "assistant" || role === 2) && hasUserMessage) {
          // Found AI response after user message - this means we have a conversation
          return true;
        }
      }
  ...
  // Helper function to start a new chat with a specific model
  const startNewChatWithModel = useCallback(
    async (
      modelName: string,
      navigate?: (path: string, options?: any) => void
    ) => {
      ...
      try {
        ...
        // Navigate to new chat page if navigate function is provided
        if (navigate) {
          navigate("/chat/new");
        }
      } catch (error) {
        console.error("Failed to start new chat with model:", error);
      } finally {
        setLoading(false);
      }
    },
