...
const UpdateDocumentTypePage = () => {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<UpdateDocumentTypeData>();

  const {
    data: documentTypeData,
    isLoading,
    isError,
  } = useDocumentType(documentTypeId!);
  ...
  }, [documentTypeData, reset]);

  const onSubmit = (data: UpdateDocumentTypeData) => {
  ...
  if (isLoading) {
    return (
      <div className="relative z-10 flex-1 overflow-auto">
        <Header title="Update Document Type" />
  ...
  if (isError || !documentTypeData) {
    return (
      <div className="relative z-10 flex-1 overflow-auto">
        <Header title="Update Document Type" />
        <div className="flex items-center justify-center h-64">
          <p className="text-red-400">Failed to load document type data</p>
        </div>
      </div>
    );
  }
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            ...
            <h1 className="text-2xl font-bold text-gray-100">
              Update Document Type: {documentTypeData.name}
            </h1>
          </div>
