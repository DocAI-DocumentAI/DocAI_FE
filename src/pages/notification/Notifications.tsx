...
import { Layout, Typography, Card, List, Tag, Row, Col, Badge, Avatar, Empty, Spin, Modal } from "antd"
import {
    BellOutlined,
    CheckCircleOutlined,
    CloseCircleOutlined,
...
import { getUserNotifications, dismissNotification, Notification, UserNotificationFilters } from "../../lib/api/notification"
...
const notificationTypes = {
    1: { label: 'Document Approved', color: 'green', icon: <CheckCircleOutlined />, bgColor: '#f6ffed' },
    2: { label: 'Document Rejected', color: 'red', icon: <CloseCircleOutlined />, bgColor: '#fff2f0' },
    3: { label: 'Document Submitted', color: 'blue', icon: <SendOutlined />, bgColor: '#f0f5ff' },
    4: { label: 'Review Reminder', color: 'orange', icon: <ExclamationCircleOutlined />, bgColor: '#fff7e6' },
    5: { label: 'Expiration Warning', color: 'purple', icon: <ClockCircleOutlined />, bgColor: '#f9f0ff' },
};
...
export default function UserNotifications() {
    ...
    const getNotificationTypeConfig = (type: number) => {
        return notificationTypes[type as keyof typeof notificationTypes] ||
            { label: `Type ${type}`, color: 'default', icon: <BellOutlined />, bgColor: '#fafafa' };
    };
    ...
    const handleNotificationClick = async (notification: Notification) => {
        ...
        try {
            ...
            setNotifications(prev => 
                prev.map(n => n.id === notification.id ? { ...n, isDismissed: true } : n)
            );
        } catch (error: any) {
                            ...
                            <List
                                ...
                                renderItem={(notification) => {
                                    ...
                                    return (
                                        <List.Item
                                            style={{
                                                backgroundColor: isUnread ? typeConfig.bgColor : '#ffffff',
                                                border: isUnread ? `1px solid ${typeConfig.color}` : '1px solid #f0f0f0',
                                                borderRadius: '8px',
                                                marginBottom: '8px',
                                                padding: '16px',
                                                cursor: 'pointer',
                                            ...
                                            <List.Item.Meta
                                                ...
                                                title={
                                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                        <div>
                                                            <Text strong={isUnread} style={{ fontSize: '16px' }}>
                                                    ...
                                                    <div>
                                                        ...
                                                        {notification.documentId && (
                                                            <div style={{ marginTop: 4 }}>
                                                                <Text style={{ fontSize: '11px', fontFamily: 'monospace', color: '#666' }}>
                                                                    Document: {notification.documentId.substring(0, 8)}...
                                                                </Text>
                                                            </div>
                                                        )}
                                                    </div>
                ...
                title={
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        {selectedNotification && (
                            <>
                                {getNotificationTypeConfig(selectedNotification.notificationType).icon}
                ...
                {selectedNotification && (
                    <div>
                        <div style={{ marginBottom: 16 }}>
                            ...
                            <div style={{ marginTop: 8 }}>
                                <Tag color={getNotificationTypeConfig(selectedNotification.notificationType).color}>
                                    {getNotificationTypeConfig(selectedNotification.notificationType).label}
                                </Tag>
                        ...
                        <Row gutter={16}>
                            ...
                            <Col span={12}>
                                <div style={{ marginBottom: 12 }}>
                                    <Text type="secondary" style={{ fontSize: '12px' }}>Sent At:</Text>
                                    <div>
                                        <Text>
                        ...
                        {selectedNotification.documentId && (
                            <div style={{ marginBottom: 12 }}>
                                <Text type="secondary" style={{ fontSize: '12px' }}>Related Document:</Text>
                                <div style={{ 
                                    marginTop: 4,
                                    padding: '8px',
                        ...
                        <div style={{ marginBottom: 12 }}>
                            <Text type="secondary" style={{ fontSize: '12px' }}>Recipient:</Text>
                            <div>
                        ...
                        <Row gutter={16}>
                            ...
                            <Col span={12}>
                                <div>
                                    <Text type="secondary" style={{ fontSize: '12px' }}>Read Status:</Text>
                                    <div>
                                        {selectedNotification.isRead ? (
                        ...
                        {selectedNotification.errorMessage && (
                            <div style={{ marginTop: 16 }}>
                                <Text type="secondary" style={{ fontSize: '12px' }}>Error Message:</Text>
                                <div style={{ 
                                    marginTop: 4,
                                    padding: '8px',
