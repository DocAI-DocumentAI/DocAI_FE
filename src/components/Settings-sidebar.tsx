        ...
        {menuItems.map((item, index) => {
          if (item.isHeader) {
            return (
              <div
                key={index}
                className="pt-2 mt-6 text-xs font-semibold text-gray-400 uppercase"
              >
                {item.title}
              </div>
            );
          ...
          return (
            <div key={index} className="relative flex gap-10 ">
              {isActive && (
                <div className="absolute left-0 w-1 h-full bg-blue-800 rounded-full"></div>
              )}
              <Link
                to={item.href}
                className={` flex items-center rounded-md px-3 w-full mx-2 py-2 text-sm ${
                  isActive
